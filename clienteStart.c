...
// Initial maximum value for auto-generated transactions (N)
...
int main(int argc, char *argv[]) {
    ...
    // --- Configuration for automatic mode: list of peer IPs to choose from ---
    // Adjust this list for your lab environment. These are example IPs.
    const char *peer_ips[] = {
        "10.1.1.2",
        "10.1.1.3",
        "10.1.1.4"
    };
    const size_t peer_count = sizeof(peer_ips) / sizeof(peer_ips[0]);

    // seed RNG for automatic mode
    srand((unsigned)(time(NULL) ^ getpid()));

    // reading loop: read commands from stdin (no prompt)
    uint32_t seqn = 1;
    char line[256];
    while (running && fgets(line, sizeof(line), stdin) != NULL) {

        // --- automatic mode trigger: if the user types exactly "start" (with optional newline) ---
        int automatic_mode = 0;
        // trim leading spaces
        char *p = line;
        while (*p == ' ' || *p == '\t') p++;
        if (strncmp(p, "start", 5) == 0 && (p[5] == '\n' || p[5] == '\0' || p[5] == ' ' || p[5] == '\t')) {
            automatic_mode = 1;
        }

        if (automatic_mode) {
            // send 10 automatic requests choosing dest and value randomly (repeats allowed)
            for (int i = 0; i < 10 && running; i++) {
                // pick random peer IP from peer_ips[]
                const char *dest_str = peer_ips[rand() % peer_count];
                uint32_t value = (uint32_t)( (rand() % INITIAL_N) + 1 ); // [1 .. INITIAL_N]

                struct in_addr dest_in;
                if (inet_pton(AF_INET, dest_str, &dest_in) != 1) {
                    // skip if invalid (shouldn't happen with hardcoded list)
                    continue;
                }
                ...
                while (running && !acknowledged) {
                        ...
                        if (r >= 2) {
                            ...
                            if (rtype == TYPE_REQ_ACK && r >= 10) {
                                ...
                                if (ack_seqn == seqn) {
                                    ...
                                    // server indicates last processed is smaller: resend (will loop)
                                    char tstamp[64];
                                    timestamp_now(tstamp, sizeof(tstamp));
                                    enqueue_message("%s server ack last %u (we sent %u) -> resending",
                                                    tstamp, ack_seqn, seqn);
                                } else {
                                    // ack_seqn > seqn (unexpected) - advance
                                    char tstamp[64];
                                    timestamp_now(tstamp, sizeof(tstamp));
                    ...
                    // timeout: loop resends automatically
                }
            }
            continue; // wait for next stdin input
        }

        // --- ORIGINAL MANUAL COMMAND PROCESSING ---
        char dest_str[64];
        uint32_t value;
